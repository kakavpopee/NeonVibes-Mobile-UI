...
-- Global reopen icon (appears when GUI is closed)
local reopenIcon

local function createReopenIcon()
    if reopenIcon then return end
    
    reopenIcon = Instance.new("TextButton")
...
function NeonVibes:Keybind(keyName)
...
function NeonVibes:Hide()
    ScreenGui.Enabled = not ScreenGui.Enabled
end

function NeonVibes:Destroy()
    if keybindConnection then keybindConnection:Disconnect() end
    ScreenGui:Destroy()
end
...
function NeonVibes:CreateWindow(name)
    ...
    function Elements:Label(text, rainbow)
    ...
    function Elements:Dropdown(text, buttons, callback, selective)
    ...
    function Elements:ColorPicker(name, default, callback)
        ...
        -- Full HSV picker UI (simplified from Turtle)
        local PickerFrame = Instance.new("Frame")
        PickerFrame.Size = UDim2.new(1, 0, 0, 200)
        PickerFrame.Position = UDim2.new(0, 0, 1, 0)
        PickerFrame.BackgroundColor3 = CONFIG.BgSecondary
        PickerFrame.Visible = false
        PickerFrame.Parent = ColorPicker
        
        -- Canvas, Hue slider, Rainbow toggle (implement as needed)
        -- ... (add HSV logic from your original Turtle code here)
